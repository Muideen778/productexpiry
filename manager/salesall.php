	 ...
	 ?>
      ...
      <div id="page-wrapper">
          ...
          <div class="col-lg-12">
            ...
            <div class="col-lg-6">
              <div style="font-size:18px; font-weight:bold">CHECK PRODUCT PANEL</div>
...
$itemstatu = "2025:02:03";//date('my', strtotime($rx['xm'].":".$rx['xy'])); //strtodate(date('m:y'), $rx['xm'].":".$rx['xy']); 
$expiredat="20".$rx['xy']."-".$rx['xm']."-30";
$expiredate=secondsToTime(strtotime($expiredat));
$tody=secondsToTime(strtotime(date("Y-m-d")));


$interval=$expiredate-$tody;

if($interval<1){
  $itemstatus= "Expired"; 
  $exp =  round($interval/30);
  $newe = str_replace('-', '', $exp);
  $iteminteval= "Item Expired ".$newe." Month Ago";  
$color = '#FF4500';
}elseif($interval < 60  and $interval>1  ){
  $itemstatus= "Almost Expired";
  $exp =  round($interval/30);
  $newe = str_replace('-', '', $exp);
$sind=$interval/2880;
//$hrd=$hr*24;
  $iteminteval= "Expire Next Month"; 
  $color = '#FFA500';     
}else{
  $itemstatus= "Not Expire"; 
  $exp =  round($interval/30);
  if($exp>12){
    $exp = $exp / 12;
    $yearr = intval($exp);
    $newe = number_format($exp,0,1);
    $n = $exp;
$whole = floor($n);      // 1
//$newe = $n - $whole*1; // .25
$newe = substr(str_replace('0.', '', $n - $whole), 0,1);

  } 
  $iteminteval= "Item Remain ".$newe." Month, ".$yearr. " Year to Expiry";    
$color = '#008B8B';
}
...
<font color="green"><?php echo $er; ?></font>
                  ...
                  <td bgcolor="<?php echo $color  ;?>" style="color:#FFF">
                    Expiring Date: <?php echo ($rx['xm'].":".$rx['xy']); ?><br>
                    <font>Interval: <?php echo $iteminteval; ?></font><br>
                    <font>Item Status: <?php echo $itemstatus; ?></font>
          ...
          <table class="table table-bordered" >
                  ...
                  -->
                      <button type="button" id="txt6" style="width:100%; height:50px" class="btn btn-default"  data-toggle="modal" da ta-target="#modal-checkout" disable></button>
